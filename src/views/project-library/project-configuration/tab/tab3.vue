<template>
  <div class="h100 border-d9 border-radius-5">
    ...
    <el-dialog title="查看模板" :visible.sync="dialogFormVisible" :modal-append-to-body="false" :close-on-click-modal="false" width="620px">
      <div class="border-bottom-06">
        <p class="fs-14 text-grey-0 fw-bold mb-10">基本信息</p>
        <el-form :model="form">
          <el-form-item label="模板名称：" :label-width="formLabelWidth">
            <el-input v-model="form.name" autocomplete="off" disabled placeholder="请输入" />
          </el-form-item>
          <el-form-item label="模板描述：" :label-width="formLabelWidth">
            <el-input v-model="form.desc" type="textarea" resize="none" disabled placeholder="请输入" autocomplete="off" />
          </el-form-item>
        </el-form>
      </div>
      <div class="p-20">
...
export default {
  ...
  methods: {
    ...
    edit(params) {
      this.$router.push({ path: '/project-library/project-configuration/model-edit-template', query: { projectName: getUrlParams().projectName, from: '模型注册', modelName: params.modelName, modelDesc: params.modelDesc }})
    }
  }
